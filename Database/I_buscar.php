<?php
 include('../CONFIG/conexion.php');  
    ...
    $query="SELECT id_indi, CONCAT(nombre1, ' ' ,nombre2, ' ',apellido1, ' ', apellido2) AS full_name ,num_doc, E3Finalizado FROM individuo WHERE  id_familia = '$id_hogar' ORDER BY created_at DESC LIMIT 20" ;

    if (isset($_POST['buscar'])) {
     $buscar_i = mysqli_real_Escape_string ($con,$_POST['buscar']);
     $query="SELECT id_indi, CONCAT(nombre1, ' ' ,nombre2, ' ',apellido1, ' ', apellido2) AS full_name ,num_doc, E3Finalizado FROM individuo WHERE id_familia = '$id_hogar' and (num_doc LIKE '%$buscar_i%' OR CONCAT(nombre1, ' ' ,nombre2, ' ',apellido1, ' ', apellido2)  LIKE '%$buscar_i%' ) ORDER BY created_at DESC LIMIT 20"; 
    }    
