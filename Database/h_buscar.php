<?php
 include('../CONFIG/conexion.php');  
    ...
    $query="SELECT * FROM familia WHERE  id_vivienda = '$id_vivienda' LIMIT 20" ;

    if (isset($_POST['buscar'])) {
     $buscar_u = mysqli_real_Escape_string ($con,$_POST['buscar']);
     $query="SELECT * FROM familia WHERE id_vivienda = '$id_vivienda' and (id_familia LIKE '%$buscar_u%' OR referencia  LIKE '%$buscar_u%') LIMIT 20"; 
    }    
   ...
   if ($fila ) {
      while ($row_u=mysqli_fetch_array($resul_u)) {
         ...
         $encuest = "<li title='Llenar encuesta Hogar' class='material-icons '><a href='create_hogar.php?id_hogar=".$row_u['id_familia']."' class='hoverable modal-trigger brown-text text-lighten-2'>poll</a></li>";  
           $tabla_u.="          
               <tr>
