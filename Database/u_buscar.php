<?php
 include('../CONFIG/conexion.php');  
    ...
    // BUSCAR PERSONA
    $tabla_u= "";
    $query="SELECT * FROM user LIMIT 15" ;

    if (isset($_POST['dato'])) {
     $buscar_u = mysqli_real_Escape_string ($con,$_POST['dato']);
     $query="SELECT * FROM user WHERE (iden LIKE '%$buscar_u%' OR name_u  LIKE '%$buscar_u%' OR last_name LIKE'%$buscar_u%' OR mail LIKE '%$buscar_u%') "; 
    }    
